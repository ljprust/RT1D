...
double TempFromBeta3Gamma4(double beta3, double gamma4) {
  double temp;
  double beta = pow(beta3, 1.0/3.0);
  double gamma = pow(gamma4, 0.25);
  double epsilon = beta/gamma;
  double epsilon3 = beta*beta*beta/gamma/gamma/gamma;
  double delta = 1.0/epsilon;
  if( epsilon < 1.0 ) {
...
void cons2prim( double * cons , double * prim , double GMr , double dV ){
   ...
   double beta3 = 3.0/2.0*rho*kB/mu/mProton/ar;
